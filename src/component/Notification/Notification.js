...
function Notification() {
  ...
  return (
    <>
      {!user ? <NeedLogin title='Notifikasi'/> : 
      <div className='notification'>
        <h1>Notifikasi</h1>
        {loading && Array.from(new Array(10)).map((s,i)=>(
          <div key={i}>
            <div className='notification-wraper'>
              ...
              <div className='notification-detail'>
                <Skeleton className={classes.skeleton} animation="wave" height={20} width="250px" style={{marginTop: '5px'}}/>
                <Skeleton className={classes.skeleton} animation="wave" height={20} width="130px" style={{marginTop: '5px'}}/>
                <Skeleton className={classes.skeleton} animation="wave" height={10} width="80px" style={{marginTop: '5px'}}/>
              </div>
            </div>
          </div>
        ))}

        {!loading && data.length < 1 ? <h1>Tidak ada notifikasi</h1> : data.map((d,i)=>(
          <div key={i}>
            <Link to={d?.link} className='notification-wraper'>
              <Avatar className={classes.root} src={d?.sender?.profilePicture}/>
              <div className='notification-detail'>
                <p>{d?.message}</p>
                <p className='notification-sender'>{d?.sender?.username}</p>
